...
private struct SetTimerView: View {
    ...
    fileprivate var body: some View {
        VStack {
            ...
            TimerCrateBtnView(timerViewModel: timerViewModel)
...
// MARK: - 타이머 생성 버튼 뷰
private struct TimerCrateBtnView: View {
    @ObservedObject private var timerViewModel: TimerViewModel
    ...
    fileprivate var body: some View {
        Button(
            action: {
                timerViewModel.settingBtnTrapped()
            },
            label: {
                Text("설정하기")
...
private struct TimerOperationView: View {
    ...
    fileprivate var body: some View {
        VStack {
            ZStack {
                VStack {
                    Text("\(timerViewModel.timeRemaining.fromattedTimeString)")
                        .font(.system(size: 28))
                        .foregroundColor(.customBlack)
                        .monospaced()
            ...
            HStack {
                ...
                Button(
                    action: {
                        timerViewModel.pauseOrRestartBtnTrpped()
                    },
                    label: {
